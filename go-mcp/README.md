...
[MCP](https://modelcontextprotocol.io) server exposing the Screenshot SDK as
two agent tools — `screenshot_list` and `screenshot_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/screenshot-mcp)
make build

# 2. Provide credentials via the environment
export SCREENSHOT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user screenshot \
  -- /absolute/path/to/screenshot-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./screenshot-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// screenshot_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// screenshot_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/screenshot-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user screenshot \
     -- "$PWD"/dist/*/screenshot-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `screenshot_list` and `screenshot_load` tools now appear
   in new sessions. Ask the agent to *"list domain using screenshot"*
   and it calls `screenshot_list` with `{"entity":"domain"}`.
...
```sh
export SCREENSHOT_APIKEY=sk_live_xxx            # API key
export SCREENSHOT_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `screenshot_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `screenshot_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SCREENSHOT_APIKEY` | API key sent with every request. |
| `SCREENSHOT_BASE` | Optional override of the API base URL. |
...
```sh
./screenshot-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"screenshot_load","arguments":{"entity":"dns_result","query":{"id":1}}}}'
```

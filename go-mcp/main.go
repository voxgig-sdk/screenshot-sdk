// Command Screenshot-mcp is an MCP server exposing the Screenshot
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Screenshot_list  — list records of an entity
//	Screenshot_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: SCREENSHOT_APIKEY carries the API key and
	// SCREENSHOT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SCREENSHOT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SCREENSHOT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewScreenshotSDK(opts)

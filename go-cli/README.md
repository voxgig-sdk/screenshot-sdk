...
boru-driven command-line client **and** interactive REPL for the Screenshot
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/screenshot-cli)
make build
...
./screenshot-cli load 1 dns_result            # {id:1} shorthand
./screenshot-cli load '{id:1}' dns_result       # explicit match map

# 5. Override the API base URL for a single call
SCREENSHOT_BASE=https://api.example.com ./screenshot-cli load 1 dns_result

# 6. No arguments -> interactive REPL
./screenshot-cli
screenshot> load 1 dns_result
screenshot> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/screenshot-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/screenshot-cli load 1 dns_result
   ```
...
```sh
./screenshot-cli load 1 dns_result          # scalar shorthand for {id:1}
./screenshot-cli load '{id:1}' dns_result     # explicit match map
```
...
./screenshot-cli load 1 dns_result
```
...
Run with no arguments to open a REPL (prompt `screenshot>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/screenshot-cli`. |
...
(`./screenshot-cli <expr>`) and interactively (the REPL), and expressions compose the

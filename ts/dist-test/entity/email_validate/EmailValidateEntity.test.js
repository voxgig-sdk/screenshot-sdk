...
(0, node_test_1.describe)('EmailValidateEntity', async () => {
    ...
    // `test.live.delayMs`; only sleeps when SCREENSHOT_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('SCREENSHOT_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.ScreenshotSDK.test();
        const ent = testsdk.EmailValidate();
        (0, node_assert_1.default)(null != ent);
    });
    (0, node_test_1.test)('basic', async (t) => {
        const live = 'TRUE' === process.env.SCREENSHOT_TEST_LIVE;
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set SCREENSHOT_TEST_EMAIL_VALIDATE_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let client = __1.ScreenshotSDK.test(options, extra);
    ...
    const idmapEnvVal = process.env['SCREENSHOT_TEST_EMAIL_VALIDATE_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'SCREENSHOT_TEST_EMAIL_VALIDATE_ENTID': idmap,
        'SCREENSHOT_TEST_LIVE': 'FALSE',
        'SCREENSHOT_TEST_EXPLAIN': 'FALSE',
        'SCREENSHOT_APIKEY': '',
    });
    idmap = env['SCREENSHOT_TEST_EMAIL_VALIDATE_ENTID'];
    const live = 'TRUE' === env.SCREENSHOT_TEST_LIVE;
    if (live) {
        client = new __1.ScreenshotSDK(merge([
    ...
    const setup = {
        ...
        struct,
        data: entityData,
        explain: 'TRUE' === env.SCREENSHOT_TEST_EXPLAIN,
        live,

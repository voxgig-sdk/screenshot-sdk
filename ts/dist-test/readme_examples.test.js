...
function isRunnable(code) {
    return (/new\s+ScreenshotSDK\b/.test(code) ||
        /\bScreenshotSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ScreenshotSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ScreenshotSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);

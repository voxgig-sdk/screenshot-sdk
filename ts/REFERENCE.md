# Screenshot TypeScript SDK Reference

Complete API reference for the Screenshot TypeScript SDK.
...
```ts
new ScreenshotSDK(options?: object)
```
...
#### `ScreenshotSDK.test(testopts?, sdkopts?)`
...
**Returns:** `ScreenshotSDK` instance in test mode.
...
Return the parent `ScreenshotSDK` instance.

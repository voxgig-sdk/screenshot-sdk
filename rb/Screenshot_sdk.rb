...
require_relative 'Screenshot_types'
...
class ScreenshotSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ScreenshotHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ScreenshotHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ScreenshotFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ScreenshotUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ScreenshotHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ScreenshotHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ScreenshotHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ScreenshotHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ScreenshotError.new(
        "graphql_error", "ScreenshotSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

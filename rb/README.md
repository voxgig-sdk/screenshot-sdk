...
The Ruby SDK for the Screenshot API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "Screenshot_sdk"
client = ScreenshotSDK.new(options)
```
...
```ruby
client = ScreenshotSDK.test(testopts, sdkopts)
```
...
raise a `ScreenshotError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Screenshot_sdk.rb       -- Main SDK module
...
The main module (`Screenshot_sdk`) exports the SDK class

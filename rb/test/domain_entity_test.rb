...
class DomainEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = ScreenshotSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SCREENSHOT_TEST_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["SCREENSHOT_TEST_DOMAIN_ENTID"]
  ...
  if env["SCREENSHOT_TEST_LIVE"] == "TRUE"
    ...
    client = ScreenshotSDK.new(Helpers.to_map(merged_opts))
  end

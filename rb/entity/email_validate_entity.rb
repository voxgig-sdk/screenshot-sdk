# Screenshot SDK EmailValidate entity
...
class EmailValidateEntity
  ...
  def data_set(args)
    if args
      @_data = ScreenshotHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ScreenshotHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [EmailValidate, Hash] the loaded EmailValidate; raises ScreenshotError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ScreenshotHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

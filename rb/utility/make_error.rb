...
module ScreenshotUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ScreenshotContext.new({}, nil)
    end
    op = ctx.op || ScreenshotOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ScreenshotResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ScreenshotError) ? err.msg : err.to_s
    ...
    sdk_err = ScreenshotError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ScreenshotError)

...
// Screenshot SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class ScreenshotMakeError
{
    public static function call(?ScreenshotContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ScreenshotContext([], null);
        }
        $op = $ctx->op ?? new ScreenshotOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ScreenshotResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ScreenshotError) ? $err->msg : (string)$err;
        $msg = "ScreenshotSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new ScreenshotError('', $msg, $ctx);

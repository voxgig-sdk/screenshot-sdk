...
class FtCtrl extends ScreenshotControl
{
...
class FtClient
{
    ...
    private ScreenshotUtility $utility;

    public function __construct(ScreenshotUtility $utility, array $options)
    {
    ...
    public function get_utility(): ScreenshotUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ScreenshotContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ScreenshotContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ScreenshotContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ScreenshotConfig::make_feature($name);
    ...
    private static function buildUrl(ScreenshotSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ScreenshotError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ScreenshotResponse($fetched);
        }
    ...
    private function fail(ScreenshotContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ScreenshotContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
class DomainEntityTest extends TestCase
{
    ...
    {
        $testsdk = ScreenshotSDK::test(null, null);
    ...
    public function test_stream(): void
    {
        ...
        $base = ScreenshotSDK::test($seed, null);
        $seen = iterator_to_array($base->Domain(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = ScreenshotConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = ScreenshotSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SCREENSHOT_TEST_DOMAIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function domain_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("SCREENSHOT_TEST_DOMAIN_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["SCREENSHOT_TEST_DOMAIN_ENTID"]);
    ...
    if ($env["SCREENSHOT_TEST_LIVE"] === "TRUE") {
        ...
        $client = new ScreenshotSDK(Helpers::to_map($merged_opts) ?? []);
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["SCREENSHOT_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,

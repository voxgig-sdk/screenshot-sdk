...
import (
	...
	omni "github.com/voxgig-sdk/screenshot-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ScreenshotError); is {
				out := map[string]any{
...
func omniCtx(arg any, client *sdk.ScreenshotSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ScreenshotSDK); is {
			client = live

...
module ScreenshotUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ScreenshotResponse.new({})
    ...
    if fetch_err
      ...
      response = ScreenshotResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ScreenshotResponse.new(fetched)
    else

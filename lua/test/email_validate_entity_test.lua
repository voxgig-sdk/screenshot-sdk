...
describe("EmailValidateEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set SCREENSHOT_TEST_EMAIL_VALIDATE_ENTID JSON to run live")
      return
    end
...
function email_validate_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("SCREENSHOT_TEST_EMAIL_VALIDATE_ENTID")
  ...
  if env["SCREENSHOT_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["SCREENSHOT_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,

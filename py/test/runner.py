...
class ScreenshotTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ScreenshotTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ScreenshotTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ScreenshotTestRunner.getenv("SCREENSHOT_TEST_LIVE")
        override = ScreenshotTestRunner.getenv("SCREENSHOT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ScreenshotTestRunner.getenv(key)
        ...
        explain = ScreenshotTestRunner.getenv("SCREENSHOT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SCREENSHOT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ScreenshotTestRunner._test_control is not None:
            return ScreenshotTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ScreenshotTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ScreenshotTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ScreenshotTestRunner.load_test_control()
...
def load_env_local():
    ScreenshotTestRunner.load_env_local()


def env_override(m):
    return ScreenshotTestRunner.env_override(m)


def entity_data(v):
    return ScreenshotTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ScreenshotTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ScreenshotTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ScreenshotTestRunner.load_test_control()


def live_client_options():
    return ScreenshotTestRunner.live_client_options()


def live_delay_ms():
    return ScreenshotTestRunner.live_delay_ms()
